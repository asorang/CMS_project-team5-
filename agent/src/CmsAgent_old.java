package src;// OSHI 라이브러리 (운영체제 및 시스템 정보)
...
public class CmsAgent_old {
    ...
    // Server Socket (시작/재시작 제어용으로 빼둠...)
    static ServerSocket serverSocket;

    // Agent UI 관련 변수 (시스템 트레이 아이콘 및 UI)
    static AgentSettingUI settingUI;
    static TrayIcon trayIcon;
    static Image iconOnline  = Toolkit.getDefaultToolkit().getImage(CmsAgent.class.getResource("/resource/tray_online.png"));
    static Image iconOffline = Toolkit.getDefaultToolkit().getImage(CmsAgent.class.getResource("/resource/tray_offline.png"));
    static MenuItem statusItem = new MenuItem();
    ...
    // Socket 열기 (Server Start)
    static void startSocket() {
        new Thread(() -> {
            try {
                serverSocket = new ServerSocket(PORT);
                System.out.println("소켓 수신 시작: " + PORT);
                while (true) {
    ...
    static void handleConnection(Socket socket){
        try{
            ...
            // 인증 성공 시 트레이아이콘 및 트레이 텍스트도 변경한다
            trayIcon.setImage(iconOnline);
            statusItem.setLabel("Connected: " + socket.getInetAddress().getHostAddress());
            ...
            trayIcon.setImage(iconOffline);         // 오프라인 상태 -> 트레이 아이콘 다시 오프라인 아이콘으로 변경
            statusItem.setLabel("... Waiting for Manager ...");
        }
    }
    ...
    static void setupSettingUI() {
        String[] names = new String[3];
        String[] paths = new String[3];
        for (int i = 0; i < Math.min(PROGRAMS.size(), 3); i++) {
            names[i] = PROGRAMS.get(i)[0];
            paths[i] = PROGRAMS.get(i)[1];
        }
        settingUI.setInitialData(String.valueOf(PORT), PASSWORD, names, paths);
        ...
        // 취소 버튼
        settingUI.getCancelButton().addActionListener(e -> {
            settingUI.setVisible(false);
        });
    }
    ...
    static void initTray(){
        ...
        statusItem = new MenuItem("... Waiting for Manager ...");
